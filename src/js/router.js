...
export class SPARouter {
  ...
  init() {
    // ── Absolute paths so Vite's dev server and build both resolve correctly
  ...
  /**
   * Register lazy controller loaders.  Called from main.js.
  ...
  _showNotFoundPage() {
    ...
    app.innerHTML = `
      <div style="text-align:center;padding:4rem 2rem;color:var(--text-secondary);">
        ...
        <a href="/" data-router-link style="color:var(--primary-color);text-decoration:none;">← Volver al inicio</a>
      </div>`;
  }
  ...
  updateMetaTags(route) {
    ...
    const meta = {
      ...
      '/': {
        ...
        ogDesc:      'Explorá proyectos, habilidades y contacto.',
        url:         baseUrl,
      },
    };

    const cfg = meta[route] ?? meta['/'];
